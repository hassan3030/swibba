...
import { useCategoryChain } from "./use-category-chain"
import { useLocationService } from "./use-location-service"
import { useMediaUpload } from "./use-media-upload"
import { useAiEstimation } from "./use-ai-estimation"
import { useItemForm } from "./use-item-form"
import { BasicInfoSection } from "./basic-info-section"
import { CategorySelector } from "./category-selector"
import { LocationSection } from "./location-section"
import { DescriptionSection } from "./description-section"
import { AllowedCategoriesSection } from "./allowed-categories-section"
import { MediaUploadSection } from "./media-upload-section"
import { AiEstimationSection } from "./ai-estimation-section"
...
export function ItemAdd() {
  ...
  const location = useLocationService(toast, t, (field: string, value: any) => {
    form.setValue(field as any, value)
  })

  const media = useMediaUpload(toast, t, () => form.getValues())

  const ai = useAiEstimation(toast, t, () => form.getValues(), location.geo_location, media.images)

  const { form, isSubmitting, handleSubmit } = useItemForm(
    t,
    toast,
    user,
  ...
  const getStep3Missing = () => {
    const missing = []
    if (media.images.length === 0) missing.push(t("Images") || "Images")
