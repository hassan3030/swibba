...
export const useAiEstimationUpdate = (
  toast: any,
  t: (key: string) => string,
  formGetValues: any,
  geo_location: any,
  newImages: File[],
  existingImages: Array<{ fileId: string; url: string; type: string }>,
  initialValueEstimate?: number
) => {
  const [aiPriceEstimation, setAiPriceEstimation] = useState<number | null>(initialValueEstimate || null)
  ...
  const requestAiPriceEstimate = async () => {
    ...
    try {
      ...
      const aiResponseResult = await sendMessage(aiInputMessage, AI_SYSTEM_PROMPT, 3, 1000)

      // Check if AI request was successful
      if (!aiResponseResult.success) {
        const errorMsg = 'error' in aiResponseResult ? aiResponseResult.error : 'AI request failed'
        throw new Error(
          errorMsg || t('AIrequestfailedafterallretryattempts') || 'AI request failed after all retry attempts'
        )
      }

      let jsonString = aiResponseResult.text
      ...
      const attemptCount = 'attempt' in aiResponseResult ? aiResponseResult.attempt : ('attempts' in aiResponseResult ? aiResponseResult.attempts : 1)
      if (attemptCount > 1) {
        toast({
          title: t('success') || 'Success',
          description: `${t('AIpriceestimationsuccessfulafter') || 'AI price estimation successful after'} ${attemptCount} ${t('attempts') || 'attempts'}!`,
          variant: 'default',
        })
      }
