...
import {
  ...
  deleteMessageByOfferId,
  getMessagesByOfferId,
  deleteChatsByOfferId,
} from "@/callAPI/swap";
    ...
    try {
      ...
      const offersWithPartner = await Promise.all(
        allOffers.map(async (offer) => {
          ...
          return {
            ...
            partner_name:
              partnerUser.success && partnerUser.data
                ? `${partnerUser.data.first_name} ${
                    partnerUser.data.last_name || ""
                  }`
                : `${t("Unknown") || "Unknown"}`,
            ...
            //   : "/placeholder.svg",
            partner_verified:
              (partnerUser.success && partnerUser.data?.verified) || false,
          };
        })
      );
      ...
      const sortedOffers = [...offersWithPartner].sort((a, b) => {
        const aTime = new Date(
          a?.date_updated || a?.date_created || 0
        ).getTime();
        const bTime = new Date(
          b?.date_updated || b?.date_created || 0
        ).getTime();
        return bTime - aTime;
      });
  ...
  const handleRemoveCompletedOffer = async (offerId) => {
    ...
    if (response.success) {
      toast({
        ...
        description:
          t("Offerremovedsuccessfully") || "Offer removed successfully",
      });
    } else {
      toast({
        title: t("error") || "Error",
                  ...
                  {filteredOffers.length === 0 ? (
                    ...
                    >
                      ...
                      <p className="text-muted-foreground text-sm">
                        {t("StartSwappingToSeeMessages") ||
                          "Start swapping items to see your conversations here"}
                      </p>
                    </motion.div>
                  ) : (
                          ...
                          <Card
                            className={`cursor-pointer transition-all duration-200 mt-1 mx-1 border border-none rounded-full ${
                              selectedOffer?.id === offer.id
                                ? "ring-2 ring-primary/30 bg-primary/20"
                                : offer.status_offer === "completed"
                                ? "bg-primary/70 dark:bg-green-900/20 border-primary/30 dark:border-primary/30"
                                : "hover:bg-primary/20"
                            }`}
                                ...
                                <div className="flex items-center text-sm w-full ">
                                  ...
                                  <div className="flex flex-col ml-2 flex-1 min-w-0">
                                    <div className="flex items-center justify-between">
                                      <span className="px-1 text-black dark:text-white capitalize truncate">
                                        {offer.partner_name || ""}
                                      </span>
                                      {/* {offer.status_offer === "completed" && (
                                     <motion.div
                                    ...
                                    <span className="px-1 text-gray-400 truncate">
                                      {offer.last_message || ""}
                                    </span>
                                  </div>
                                </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {selectedOffer?.status_offer === "completed" && (
                        <Button
                          ...
                          onClick={async () => {
                            try {
                              const res = await deleteChatsByOfferId(
                                selectedOffer.id
                              );
                              if (res?.success) {
                                toast({
                ...
                <div className="flex flex-col max-h-[calc(100vh-250px)]">
                  ...
                  <ScrollArea className="flex-1 p-4 overflow-y-auto [&_[data-radix-scroll-area-scrollbar]]:hidden">
                    ...
                    >
                      ...
                      {selectedOffer &&
                        (selectedOffer.status_offer === "completed" ||
                          selectedOffer.status_offer === "rejected") && (
                          <motion.div
                            className={`${
                              selectedOffer.status_offer === "completed"
                          ...
                          >
                            ...
                            <div className="text-xs text-muted-foreground mt-1">
                              {selectedOffer.status_offer === "completed"
                                ? t(
                                    "Thisoffercannotbechanged_anychatremainsvisible"
                                  ) ||
                                  "This offer is completed and cannot be changed. Chat remains visible."
                                : t("Thisoffercannotbechanged_itwasrejected") ||
                                  "This offer was rejected and cannot be changed."}
                            </div>
                          </motion.div>
                        )}
                      ...
                      {selectedOffer &&
                        ...
                        ["pending", "accepted"].includes(
                          String(selectedOffer.status_offer)
                        ) && (
                          <motion.div
                          ...
                          >
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 items-start">
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                  {offerItems
                                    .filter(
                                      (i) =>
                                        i.offered_by ===
                                        selectedOffer.from_user_id
                                    )
                                    .slice(0, 4)
                                    .map((i) => {
                                      const img =
                                        i?.images?.[0]?.directus_files_id?.id;
                                      const url = img
                                        ? `${mediaURL}${img}`
                                        : "/placeholder.svg";
                                      return (
                                        <div
                                          key={`${i.offer_item_id}-${i.id}`}
                                          className="flex items-center gap-2 bg-card/50 rounded-md p-2"
                                        >
                                          <img
                                            src={url}
                                            alt={i.name || "item"}
                              ...
                              <div>
                                <h4 className="text-sm font-semibold mb-2">
                                  {t("Receiveritems") || "Receiver items"}
                                </h4>
                                <div className="grid grid-cols-2 gap-2">
                                  {offerItems
                                    .filter(
                      ...
                      {selectedOffer &&
                        myUserId === selectedOffer.to_user_id && (
                          <motion.div
                            variants={chatMessageVariants}
                            initial="hidden"
                            animate="visible"
                          ...
                          >
                            ...
                            >
                              <div className="text-sm">
                                {t("WelcomeReceiverIntro") ||
                                  "Welcome! You’ve received a new swap offer. You can review details, confirm items, or ask questions here."}{" "}
                              ...
                              {!(
                                ...
                                <div className="mt-2">
                                  <Link
                                    href={`/recived-items#${selectedOffer.id}`}
                              ...
                              <div className="text-xs opacity-70 mt-2">
                                {t("OfferTips") ||
                                  "Tip: Check included items, propose changes, or proceed to confirm when ready."}
                              </div>
                            </motion.div>
                          </motion.div>
                        )}
              ...
              >
                <div className="text-center">
                  ...
                  <p className="text-muted-foreground">
                    {t("Chooseaswapoffertostartchatting") ||
                      "Choose a swap offer to start chatting"}
                  </p>
                  <Link href="/products" className=" text-2xl ">
                    <Button
                      variant="link"
                      className="mt-4 hover:no-underline text-primary/90 hover:text-primary/80 border border-primary/90 px-4 py-2 border-primary/90 hover:scale-105 transition-transform"
                    >
                      {t("makeNewOfferToStartChatting") ||
                        "Make a new offer to start chatting"}
                    </Button>
                  </Link>
                </div>
              </motion.div>
